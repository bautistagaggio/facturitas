...
export abstract class apiHelper {
    ...
    static async getComprobanteByType(afip: Afip, numero_de_comprobante: number, punto_de_venta: number, tipo_de_comprobante: number) {
        try {    
            const lastVoucher = await afip.ElectronicBilling.getVoucherInfo(numero_de_comprobante, punto_de_venta, tipo_de_comprobante);
            return lastVoucher;
    } catch (error) 
    {
    ...
    static async createVoucher(afip: Afip) {
